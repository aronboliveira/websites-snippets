...
function addGPTAltEnter() {
  const hd = kEv => {
    ...
    if (!(kEv.key?.toLowerCase().trim() == "enter" || kEv.keyCode === 13))
      return;
...
function addProseFocusKb() {
  const handleProse = async kEv => {
    ...
    if (
      !(
        prose?.isConnected &&
        typeof prose.focus === "function" &&
        typeof prose.click === "function"
      )
    )
      return;
    prose.focus();
  ...
  window.removeEventListener("keyup", handleProse);
  window.addEventListener("keyup", handleProse);
}

function addAllGptKbs() {
  let id = "",
    id2 = "",
    shouldTryHover = true,
    pageCase = 1;
  ...
  for (const cb of [handleProse, hd, openDDHandle, choseModelHandle]) {
